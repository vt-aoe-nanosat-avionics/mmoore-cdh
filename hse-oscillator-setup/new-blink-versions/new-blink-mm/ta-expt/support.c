...
void init_clock_hse(void) {
    ...
    rcc_set_ppre1(RCC_CFGR_PPRE1_DIV2);
    ...
    //usart_send_blocking(USART1, '7');
    rcc_osc_on(RCC_PLL);
    //usart_send_blocking(USART1, '8');
    rcc_wait_for_osc_ready(RCC_PLL);
    //usart_send_blocking(USART1, '9');
...
/*void init_clock_hse(void) {
    rcc_osc_on(RCC_HSE);                    // 16 MHz external oscillator
...
void init_clock_lse(void) {
    rcc_osc_on(RCC_LSE);                    // 16 MHz external oscillator
    gpio_mode_setup(GPIOH, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO0 | GPIO1);
    
    rcc_wait_for_osc_ready(RCC_LSE);        // Wait until oscillator is ready
    rcc_set_sysclk_source(RCC_CFGR_SW_HSE); // Sets sysclk source for RTOS        doesn't have LSE interpretation, so everything after here probably needs to be changed.
    rcc_set_hpre(RCC_CFGR_HPRE_NODIV);        // AHB at 80 MHz (80 MHz max.)
    rcc_set_ppre1(RCC_CFGR_PPRE1_DIV2);       // APB1 at 40 MHz (80 MHz max.)
    rcc_set_ppre2(RCC_CFGR_PPRE2_NODIV);      // APB2 at 80 MHz (80 MHz max.)
    //flash_prefetch_enable();                  // Enable instr prefetch buffer
...
void init_lpuart(void) {
    rcc_periph_reset_pulse(RST_LPUART1);
    rcc_periph_clock_enable(RCC_GPIOB);
    rcc_periph_clock_enable(RCC_LPUART1);
    gpio_mode_setup(GPIOB, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO11 | GPIO10);
    gpio_set_af(GPIOB, GPIO_AF8, GPIO11);  // LPUART1_TX is alternate function 8
    gpio_set_af(GPIOB, GPIO_AF8, GPIO10); // LPUART1_RX is alternate function 8
    usart_set_baudrate(LPUART1, 115200);
...
/*void init_rtc(void) {
    rcc_osc_on(RCC_HSE);               // Low-speed internal oscillator
    rcc_wait_for_osc_ready(RCC_HSE);   // Wait until oscillator is ready
    rcc_periph_clock_enable(RCC_PWR);  // Enable power interface clock for the RTC
    pwr_disable_backup_domain_write_protect();
    rcc_set_rtc_clock_source(RCC_HSE); // Set RTC source
    rcc_enable_rtc_clock();            // Enable RTC
    rtc_wait_for_synchro();
    pwr_enable_backup_domain_write_protect();
    rtc_set = 0;                       // RTC date and time has not yet been set
...
void rx_lpuart1(rx_cmd_buff_t* rx_cmd_buff_o) {
    while (                                             // while
        usart_get_flag(LPUART1, USART_ISR_RXNE) &&          //  USART1 RX not empty AND
        rx_cmd_buff_o->state != RX_CMD_BUFF_STATE_COMPLETE  //  Command not complete
        ) {                                                //
        uint8_t b = usart_recv(LPUART1);                  // Receive byte from RX pin
        push_rx_cmd_buff(rx_cmd_buff_o, b);              // Push byte to buffer
    }                                                  //
}

void tx_lpuart1(tx_cmd_buff_t* tx_cmd_buff_o) {
    while (                                             // while
        usart_get_flag(LPUART1, USART_ISR_TXE) &&           //  USART1 TX empty AND
        !(tx_cmd_buff_o->empty)                           //  TX buffer not empty
        ) {                                                //
        uint8_t b = pop_tx_cmd_buff(tx_cmd_buff_o);      // Pop byte from TX buffer
        usart_send(LPUART1, b);                            // Send byte to TX pin
    }                                                  //
}