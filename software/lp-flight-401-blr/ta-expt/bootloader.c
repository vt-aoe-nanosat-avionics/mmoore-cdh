...
void init_uart(void) {
  rcc_periph_reset_pulse(RST_LPUART1);      // pulse reset for LPUART1
  rcc_periph_clock_enable(RCC_GPIOB);       // enabling clock for GPIOB, the bus containing the low power pins
  rcc_periph_clock_enable(RCC_LPUART1);     // enabling clock for LPUART1, which contains all low power USART commands
  gpio_mode_setup(GPIOB,GPIO_MODE_AF,GPIO_PUPD_NONE,GPIO10|GPIO11);     // setup for GPIO mode on GPIOB for GPIO10 (pin 29, LPRX) and GPIO11 (pin 30, LPTX)
  gpio_set_af(GPIOB,GPIO_AF8,GPIO11);  // USART1_TX is alternate function 7, LPUART1_TX is alternate function 8
  gpio_set_af(GPIOB,GPIO_AF8,GPIO10); // USART1_RX is alternate function 7, LPUART1_RX is alternate function 8
  usart_set_baudrate(LPUART1,115200); // this line and below are basically the same, but changed USART1 to LPUART1
...
void rx_usart1(rx_cmd_buff_t* rx_cmd_buff_o) {
  while(                                             // while
   usart_get_flag(LPUART1,USART_ISR_RXNE) &&          //  LPUART1 RX not empty AND
   rx_cmd_buff_o->state!=RX_CMD_BUFF_STATE_COMPLETE  //  Command not complete
  ) {                                                //
    uint8_t b = usart_recv(LPUART1);                  // Receive byte from low power RX pin
...
void tx_usart1(tx_cmd_buff_t* tx_cmd_buff_o) {
  while(                                             // while
   usart_get_flag(LPUART1,USART_ISR_TXE) &&           //  LPUART1 TX empty AND
   !(tx_cmd_buff_o->empty)                           //  TX buffer not empty
  ) {                                                //
    uint8_t b = pop_tx_cmd_buff(tx_cmd_buff_o);      // Pop byte from TX buffer
    usart_send(LPUART1,b);                            // Send byte to low power TX pin
  }                                                  //
}
